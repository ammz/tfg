...
La ecuación del método de Euler semi-implícito se obtiene a través de la serie de Taylor, la segunda ley de Newton y la idea conceptual que las fuerzas se mantienen para el siguiente paso de tiempo.

A continuación realizaremos una serie de pasos para demostrar cómo se llega a la ecuación del método Euler semi-implícito:

Partimos de la conocida ecuación de la segunda ley de Newton que estable que las aceleraciones que experimenta un cuerpo son proporcionales a las fuerzas que le afectan:
\[
\mathbf{F} = m \cdot \mathbf{a}
\]
El otro instrumento matemático que utilizaremos es la serie de Taylor, que nos permite aproximarnos a las funciones mediante una serie de potencias o suma de potencias enteras de polinomios que se prolonga hasta el infinito, donde cada uno de los sumandos está elevado a una potencia mayor al antecedente como se muestra en la siguiente ecuación:
\[
F(x) = f(a) + \dfrac{f’(a)}{1!} (x-a) + \dfrac{f’’(a)}{2!} (x-a)^2 + \dots
\]
Aplicando esto con el concepto de la integración temporal obtenemos la siguiente ecuación:
\[
F(t_{n+1}) = f(t_n) + \dfrac{f’(t_n)}{1!} (t_{n+1}-t_n) + \dfrac{f’’(t_n)}{2!} (t_{n+1} - t_n)^2 + \dots
\]
Y sabemos que la derivada de la posición es velocidad y la derivada de la velocidad es la aceleración

Podemos despejar la serie de Taylor
\[
...
\]
Despejando la aceleración obtenemos las siguientes ecuaciones:
...
s_{n+1} &= s_n + v_{n+1} \cdot \Delta t + \dfrac{(a_n \cdot \Delta t^2)}{2}
\end{align*}

Estas ecuaciones han de ejecutarse en orden, ya que dependen la posición depende de la velocidad y la aceleración y la velocidad depende de la aceleración y la aceleración se puede calcular siempre que supongamos que las fuerzas para el siguiente instante de tiempo son iguales. Esto funciona para intervalos de tiempo pequeños.
...
Si suponemos que la partícula se encuentra en movimiento en el interior del hexaedro, podemos determinar si la partícula ha colisidonado con una de las caras, cuando alguna de las coordenadas de su posición se encuentra fuera de los límites calculados anteriormente.

Análogamente, si suponemos que la partícula se encuentra en el exterior del hexaedro seguiríamos necesitando los mismos parámetros para definir nuestro hexaedro, pero esta vez la colisión se detectaría si los valores de la posición de la partícula estuvieran dentro de esos límites.
...
En este trabajo, nos centraremos en explicar el caso en el que la partícula se encuentra dentro del hexaedro, ya que nos resultará muy conveniente al permitir representar un fluido contenido en un espacio cerrado.

El algoritmo que se utilizará para determinar las colisiones deberá respetar las restricciones anteriores sintetizadas en las siguientes expresiones lógicas:
\begin{align*}
...
Finalmente, cuando estas dos condiciones se satisfagan, entonces la partícula habrá colisionado con el cilindro.

El algoritmo que se utilizará para determinar las colisiones deberá respetar las restricciones anteriores sintetizadas en las siguientes expresiones
\[
...
La colisión partícula-esfera es la más fácil de implementar, ya que sólo se tiene que comprobar si la distancia que separa la esfera y la partícula es inferior o superior, dependiendo de si la colisión es exterior o interior, a la suma de los radios de la esfera y la partícula. Así pues, los únicos parámetros que tenemos que tener en cuanta son el radio y el centro de la esfera con la que se va a colisionar. Al conocer estos parámetros solo necesitaríamos aplicar una operación vectorial que nos dijera el módulo o distancia entre nuestra partícula y el centro y una vez obtenida esa distancia calcular si es menor que el radio de la esfera, si el resultado es verdadero nuestra esfera ha colisionado con la partícula.

#+CAPTION: Colisión esfera
#+ATTR_LATEX: :width 8cm
[[./graficos/esfera.pdf]]

El algoritmo que se utilizará para determinar las colisiones deberá respetar las restricciones anteriores sintetizadas en la siguiente expresione:
\[
...
La mayoría de los modelos existentes no son figuras geométricas con alta simetría que nos permiten calcular rápidamente sus colisiones, si no que son mallas de triángulos que forman todo tipo de figuras. Nos centramos en triángulos, ya que el estándar en en el ámbito de CG es que los modelados sean mallas de triángulos por ser el polígono que mejor se adapta a contornos complejos.
...
Seguidamente explicaremos los pasos necesarios para calcular si una partícula ha colisionado con un triángulo, debido a que esta seria la operación que tendríamos que aplicar a todos los triángulos de un modelo discretizado por estos.
...
Este proceso, denominado campo de distancia, nos permite determinar si existe colisión entre la partícula y el triángulo independientemente de la orientación y posición en la que se encuentre el triángulo. Para ello, el primer paso es determinar si la partícula está a una distancia en la que es posible que colisione con el triángulo.

Para llevar a cabo esta tarea, se deben realizar una secuencia de pasos que vamos a describir a continuación:

1. Determinamos la posición de cada vértice del triángulo junto con la posición de la partícula en ese instante. Escogemos dos aristas del triángulo y calculamos el producto vectorial de esos dos vectores. Como resultado obtenemos un vector perpendicular al triángulo. El módulo de dicho vector perpendicular nos da la área del paralelogramo que definen ambos vectores, según la expresión matemática $|\mathbf{b} \times \mathbf{c}| = |\mathbf{b}||\mathbf{c}||\sin \theta|$, tal como se ilustra en la figura [[fig:triangulo01]].
...
2. Obtenemos el vector resultante de nuestra partícula hacia el vértice del que nacen nuestros vectores o aristas elegidos. Como vemos en la figura  [[fig:triangulo02]].
...
3. Calculamos el producto escalar del producto vectorial y el vector con dirección hacia la partícula. La magnitud de este producto escalar representa el volumen de nuestro trapecio imaginario.
...
\]
donde $P$ es la posición de la partícula, altura es la distancia de nuestra partícula al triángulo, superficie es la magnitud del producto vectorial que define el plano y $S$ es el producto vectorial.
...
Esencialmente, el proceso que se sigue tiene como objetivo calcular la nueva velocidad de la partícula.

una de las propiedades constitutivas de nuestra partícula que queremos modificar cuando se encuentra con una colisión,

Este cálculo se lleva a cabo mediante la realización de varias operaciones vectoriales que nos permiten conocer el vector resultante de velocidad a asignar. Además, también necesitaremos algunos parámetros adicionales como la tasa de rozamiento y rebote, ya que tendrán influencia en nuestra velocidad final.

El primer paso es obtener la velocidad incidente al detectarse la colisión. Una vez realizado esto, con el vector normal almacenado, calculamos el vector proyección de la velocidad incidente sobre el vector normal del plano además de la velocidad inicial tangencial que es la resultante de la resta de los vectores de la velocidad inicial menos la proyección de la velocidad inicial sobre el plano.

A estas dos velocidades descompuestas le aplicamos una tasa de rebote y una tasa de rozamiento, respectivamente. La suma de estos dos vectores de velocidad descompuesta da como resultado la velocidad resultante final que será la que le apliquemos a nuestra partícula una vez detectada la colisión.
...
Una vez descritas las funciones desarrolladas para este simulador vamos a presentar la estructura que sigue el bucle y las actividades que se llevan a cabo

Los pasos que sigue el bucle son:

- inicialización de las variables: definiremos las variables como objetos a colisionar y límites del espacio de nuestro simulador. En el que también inicializamos las variables como el número de partículas, posiciones velocidades y aceleraciones iniciales, así como la masa, densidad y paso de tiempo de nuestro bucle de simulación

- cálculo de vecinas: Una vez inicializada las variables del entorno a simular realizamos un calculo de las partículas vecinas, en el que le asignaremos a cada partícula un numero de vecinas usando ya sea fuerza bruta o por etiquetado hash

- colisiones: Justo después hay que detectar si las partículas han colisionado esto se hace a través de la detección de colisiones de diferentes cuerpos geométricos con alta simetría o de modelos discretizados con triángulos.

  - No colisiona: Calculamos las fuerzas internas de cada partícula con SPH, para lo que necesitamos las vecinas del método anterior, calculamos las nuevas aceleraciones de este paso de tiempo a través de la suma de fuerzas y la masa de cada partícula, con esto decimos que hemos calculado la dinámica para el fluido cuando no existe colisión

  - Colisiona: Se aplica la respuesta colisión que nos modifica la velocidad y posición de la partícula para que en el siguiente paso de tiempo no colisione.

- Método de Euler: Se calculan las nuevas velocidades y posiciones de las partículas a través de la aceleración siempre que esta no haya colisionado.

- Por último, reinicializan las variables de posición, velocidad y densidad y el bucle se repite.
...
Para mejorar el rendimiento  de este bucle de simulación se han paralelizado los siguientes procesos.

- Cálculo de vecinas: todo el calculo de vecinas se lleva a cabo paralelamente con CUDA
- La detección de colisiones y respuesta de colisiones se lleva a cabo de forma paralela para cada partícula
- El cálculo de las fuerzas del fluido se lleva a cabo de forma paralela para cada partícula
- El cálculo de las nuevas aceleraciones se lleva a cabo de manera paralela para cada partícula
- El cálculo de las nuevas posiciones, velocidades, densidad y presión también se lleva a cabo de manera paralela para cada partícula.
