...
- Búsqueda exhaustiva:
  ...
  /Búsqueda por fuerza bruta/: La búsqueda de vecinos por fuerza bruta es un algoritmo que se utiliza para encontrar las partículas más cercanas a una partícula dada en un conjunto de partículas. El algoritmo consiste en comparar la distancia entre cada partícula y la partícula dada, y luego seleccionar aquellas que estén dentro de un radio de suavizado específico.
...
- Si la distancia es menor o igual al radio de suavizado, se considera que es una partícula vecina y se añade a la lista de vecinos.
...
La búsqueda de vecinos basada en etiquetado hash es una técnica utilizada para encontrar partículas vecinas en el método de partículas suaves (SPH). Esta técnica se basa en el uso de una tabla hash para almacenar las partículas en diferentes “cubos” en función de su posición en el espacio.
...
Uno de los problemas que genera usar etiquetado hash es la coalescencia de memoria, es decir, que las direcciones de memoria que sean cercanas representen cubículos adyacentes por lo que no se perdería tanto tiempo de computo. Esto lo resolvemos siguiendo la propuesta de Perea et al. [cite:@Perea-Cordero17], realizando la partición del espacio de la siguiente manera:
...
- /fuerzaMuelle/: Este es el método que calcula las fuerzas internas de cada partícula basándose en la ley de elasticidad de Hooke. Modifica las fuerzas internas del conjunto de partículas.
...
- El cálculo de las fuerzas del fluido: (/fuerzaMuelle/, /FuncionKernel/, /GradienteKernel/, /LaplacianoKernel/, /fuerzaSPH/)
